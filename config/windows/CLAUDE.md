...
- **ReVa (Ghidra MCP)**: 디컴파일, xref, 심볼, 문자열, 콜그래프, 데이터플로우, vtable 분석
  - PWN/REV 문제 시 Ghidra가 켜져 있어야 사용 가능
  - 재부팅 후 반드시 update-reva 실행
...
| one_gadget | libc one gadget 탐색 |
| seccomp-tools | seccomp 규칙 분석 |
| ROPgadget | ROP 가젯 탐색 |
| checksec | 바이너리 보호기법 확인 |
| gdb + pwndbg | 동적 디버깅 (x86 네이티브, 맥보다 빠름) |
...
7. GDB 디버깅: gdb -batch -x 스크립트로 docker_exec에서 실행 (WSL2 x86 네이티브라 빠름)
